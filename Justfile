...
dev-frontend $force="":
    ...
    # Ensure portless proxy is running on port 1355 (no sudo needed).
    # Idempotent — if already running, just confirms it.
    portless proxy start --port 1355 --https 2>&1 | tail -1
...
backend-test exclude-tags="":
    export SMP_DB_TEST_PASSWORD=$(grep ^SMP_DB_PASSWORD= .env | cut -d= -f2-); \
