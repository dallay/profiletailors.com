...
Allow patch/safe minor only; never major, framework, auth, or database migrations. If uncertainty
...
Owns ../state/dependency-maintenance.yaml; use the framework schema and never set execution data
without an actual run.

## Report

Owns ../reports/dependency-maintenance.md; report facts, evidence, result, validation, unresolved
findings, and risks without chain-of-thought or secrets.

## Pull Request

Create exactly one Draft Pull Request with framework sections. Suggested title: chore(deps): apply
safe dependency maintenance.
