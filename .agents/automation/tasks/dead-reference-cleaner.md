...
Detect nonexistent files, directories, modules, routes, endpoints, ADRs, OpenSpec artifacts, diagrams, images, commands, scripts, anchors.
...
Correct deterministic replacements only; never guess. If uncertainty persists, record it and continue safe unrelated work.
...
Owns ../state/dead-reference-cleanup.yaml; use the framework schema and never set execution data without an actual run.

## Report

Owns ../reports/dead-reference-cleanup.md; report facts, evidence, result, validation, unresolved findings, and risks without chain-of-thought or secrets.

## Pull Request

Create exactly one Draft Pull Request with framework sections. Suggested title: docs: remove dead repository references.
