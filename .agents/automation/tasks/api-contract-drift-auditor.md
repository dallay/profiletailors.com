...
Read ../state/api-contract-drift.yaml as context and revalidate every finding.
...
Never redesign APIs; correct client drift only when backend is tested and unambiguous. If uncertainty persists, record it and continue safe unrelated work.
...
Owns ../state/api-contract-drift.yaml; use the framework schema and never set execution data without an actual run.

## Report

Owns ../reports/api-contract-drift.md; report facts, evidence, result, validation, unresolved findings, and risks without chain-of-thought or secrets.

## Pull Request

Create exactly one Draft Pull Request with framework sections. Suggested title: fix(api): reconcile verified contract drift.
