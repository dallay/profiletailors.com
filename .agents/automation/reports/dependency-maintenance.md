...
- Added `@types/node` `25.9.5` to `apps/web/admin/package.json` to resolve node type definition resolution during `vue-tsc` build checks.
...
| admin-types-node-drift | `@types/node` | missing-dev-dependency | `null` | `25.9.5` | resolved | Added `@types/node` to admin devDependencies to satisfy `vue-tsc`. |
