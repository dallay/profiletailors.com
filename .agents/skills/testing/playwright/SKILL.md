...
- **App-specific configs**: Each app has its own `playwright.config.ts` with different `baseURL` and
  `testIgnore`
...
- **Use shared fixtures**: Import `selectors`, `helpers`, `testData`, `mockResponses` from
  `../fixtures`
...
- If the test is correct but the app is broken, mark as `test.fixme()` with a comment explaining the
  issue
...
**ALWAYS check `packages/testing-e2e/tests/fixtures/index.ts`** for existing selectors before
creating new ones:
