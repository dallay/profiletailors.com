...
> **When to use**: Setting up a new project, adjusting timeouts, adding browser targets, configuring
> CI behavior, or managing environment-specific settings.
...
webServer: {
  command: 'npm run dev',
      url
:
  'http://localhost:4000/api/health',  // use real endpoint
      reuseExistingServer
:
  !process.env.CI,
      timeout
