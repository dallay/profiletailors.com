...
> **When to use**: Testing email verification, password reset, session timeout/expiration, or
> remember-me functionality. For basic auth setup (storage state, OAuth mocking, MFA, role-based
> access), see [authentication.md](authentication.md).
...
1. **Configure shorter session timeouts in test environments** — Enables testing timeout behavior
   without slow tests
...
- [authentication.md](authentication.md) — Storage state, OAuth mocking, MFA, role-based access, API
  login
