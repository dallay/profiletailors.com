...
**Use when**: Testing forms built with react-hook-form or Formik—Playwright interacts with DOM, form
...
**Use when**: Testing components rendered via `ReactDOM.createPortal()`—modals, dialogs, tooltips,
menus. These render outside parent DOM but Playwright sees the full document.
