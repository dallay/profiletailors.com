...
> **When to use**: Testing REST APIs directly — validating endpoints, seeding test data, or
> verifying backend behavior without browser overhead.
...
**Use when**: Every API has error paths — test them. A missing 401 test today is a security hole
tomorrow.
...
**Fix**: Verify the server is running before tests. Use `webServer` in config to start it
automatically.
...
**Fix**: Check `response.status()` first — a 500 or 302 often returns HTML. Log
`await response.text()` to see the actual body. Verify the `Accept: application/json` header is set.
...
**Cause**: The built-in `request` fixture does not carry browser cookies or auth tokens
automatically.

**Fix**: Set `extraHTTPHeaders` in config or create a custom authenticated fixture. If you need
cookies from a browser login, use `page.request` instead.
...
**Fix**: Use `process.env` for secrets and base URLs. Run database seeds or migrations in
`globalSetup`. Use unique identifiers (timestamps, UUIDs) for test data. Check that the CI `baseURL`
