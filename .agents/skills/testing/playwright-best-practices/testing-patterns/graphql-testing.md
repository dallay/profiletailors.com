...
All GraphQL requests go through `POST` to a single endpoint. Send `query`, `variables`, and
optionally `operationName` in the JSON body.
...
**Cause**: GraphQL servers return HTTP 200 even when the query has errors. The actual error is in
the `errors` array.
...
**Cause**: Variable names in the query don't match the `variables` object keys, or types don't
match.
