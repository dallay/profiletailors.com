...
> **When to use**: Testing form filling, submission, validation messages, multi-step wizards,
> dynamic fields, and auto-complete interactions.
...
**Use when**: Testing search fields, address lookups, mention pickers, or any input that shows
...
**Use when**: The form spans multiple pages or steps, with next/previous navigation and per-step
validation.
...
**Use when**: Testing what happens after a form is submitted — success messages, redirects, error
responses from the server, and loading states during submission.
...
**Use when**: Testing any form with standard HTML inputs — text, email, password, number, textarea,
...
**Use when**: Testing native `<input type="date">`, `<input type="time">`,
`<input type="datetime-local">`, or third-party date pickers.
...
**Use when**: Testing email format, phone number format, password strength, and business-specific
validation rules.
...
**Use when**: Testing "clear form" or "reset" functionality, verifying that fields return to their
default values.
...
**Cause**: The input field uses a contenteditable div (rich text editors), not a real `<input>` or
`<textarea>`.
...
hidden input but the UI does not update.
...
**Cause**: The validation triggers on `blur` (focus leaving the field), but `fill()` does not
trigger blur automatically.
