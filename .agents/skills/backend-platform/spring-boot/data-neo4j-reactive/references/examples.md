...
class Person {
    ...
    fun getName(): String {
        return name; }
    fun getBirthYear(): Integer {
        return birthYear; }
    public List<ActedIn> getActedIn()
    { return actedIn; }
...
interface UserRepository extends Neo4jRepository<User, String> {
    ...
    @Query(
        "MATCH (u:User {username: $username})-[:FOLLOWS*2..3]->(suggested:User) " +
    ...
    @Query(
        "MATCH (u:User {username: $username})-[:FOLLOWS]->(following)-[:POSTED]->(p:Post) " +
...
interface PostRepository extends Neo4jRepository<Post, Long> {
    ...
    @Query(
        "MATCH (p:Post)-[:TAGGED]->(h:Hashtag {tag: $tag}) " +
                "RETURN p ORDER BY p.createdAt DESC LIMIT $limit"
    )
...
interface ProductRepository extends Neo4jRepository<Product, String> {
    ...
    @Query(
        "MATCH (p:Product)-[:BELONGS_TO]->(c:Category {name: $categoryName}) " +
    ...
    @Query(
        "MATCH (p:Product {sku: $sku})-[:SIMILAR_TO]->(similar:Product) " +
    ...
    @Query(
        "MATCH (p:Product {sku: $sku})-[r:PURCHASED_WITH]->(related:Product) " +
    ...
    @Query(
        "MATCH (p1:Product {sku: $sku1})<-[:PURCHASED_WITH]-(p2:Product)" +
...
interface MovieRepository extends Neo4jRepository<Movie, String> {
    ...
    @Query(
        "MATCH (m:Movie) WHERE $genre IN m.genres " +
...
extends ReactiveNeo4jRepository < Movie, String> {
    ...
    @Query(
        "MATCH (m:Movie)-[:ACTED_IN]-(p:Person {name: $actorName}) " +
                "RETURN m ORDER BY m.releaseYear DESC"
    )
...
class ReactiveMovieRepositoryTest {
    ...
    static void initializeNeo4j()
    {
        embeddedNeo4j = Neo4jBuilders.newInProcessBuilder()
            .withDisabledServer()
            .withFixture(
                "CREATE (m:Movie {imdbId: 'tt0133093', " +
