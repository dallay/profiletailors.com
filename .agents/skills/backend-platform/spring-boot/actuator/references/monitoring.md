...
> the [API documentation](https://docs.spring.io/spring-boot/docs/current/actuator-api/htmlsingle/).
...
`management.endpoints.web.base-path` property to change the prefix for your management endpoint, as
...
The preceding example changes the endpoint from `/actuator/{id}` to `/manage/{id}` (for example,
`/manage/info`).
...
expose endpoints by using a different HTTP port.
...
> **NOTE**
>
> On Cloud Foundry, by default, applications receive requests only on port 8080 for both HTTP and
> TCP routing. If you want to use a custom management port on Cloud Foundry, you need to explicitly
> set up the application's routes to forward traffic to the custom port.

## Configuring Management-specific SSL

When configured to use a custom port, you can also configure the management server with its own SSL
by using the various `management.server.ssl.*` properties. For example, doing so lets a management
server be available over HTTP while the main application uses HTTPS, as the following property
settings show:
...
3. **Limit Exposure**: Only expose necessary endpoints (`include` specific endpoints rather than
   using `*`)
