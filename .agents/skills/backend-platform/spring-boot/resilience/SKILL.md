...
description: Use when implementing reactive fault-tolerance patterns in Spring Boot 4 with Resilience4j, including circuit breakers, retries, rate limiting, bulkheads, time limiters, and fallbacks for downstream calls.
...
Provides Resilience4j patterns (circuit breaker, retry, rate limiter, bulkhead, time limiter,
fallback) for Spring Boot 3.x fault tolerance with configuration and testing workflows.
...
1. **Circuit Breaker**: Call endpoint with failures → check `GET /actuator/circuitbreakers` shows
   `OPEN` → wait `waitDurationInOpenState` → verify state transitions to `HALF_OPEN` → `CLOSED`

2. **Retry**: Enable `resilience4j.retry.metrics.enabled: true` → invoke endpoint → verify
   `retry.{instance}.successful-calls-with-retry-attempts` metric increases

3. **Rate Limiter**: Send requests exceeding `limitForPeriod` → verify 429 status → check
   `GET /actuator/ratelimiters` shows `LIMITED`

4. **Bulkhead**: Load test with concurrent requests exceeding `maxConcurrentCalls` → verify excess
   requests fail immediately with `BulkheadFullException`

5. **Time Limiter**: Mock async delay beyond `timeoutDuration` → verify fallback triggers after
   timeout
...
- **Size bulkheads based on load**: Calculate thread pool and semaphore sizes from expected
  concurrency
...
**See also:**
[Configuration Reference](references/configuration-reference.md) · [Testing Patterns](references/testing-patterns.md) · [Examples](references/examples.md) · [Resilience4j Docs](https://resilience4j.readme.io/) · [Actuator Skill](../actuator/SKILL.md)
