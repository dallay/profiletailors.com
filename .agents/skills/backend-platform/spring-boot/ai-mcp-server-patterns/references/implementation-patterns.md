...
class ApiTools(webClientBuilder: WebClient.Builder) {
    ...
    fun callApi(
        ...
        if (!headersJson.isNullOrBlank()) {
            runCatching {
                val map =
                    ObjectMapper().readValue(headersJson, Map::class.java) as Map<String, String>
...
class CustomMcpConfig {
    ...
    @Bean
    fun mcpServerCustomizer(meterRegistry: MeterRegistry): McpServerCustomizer =
        McpServerCustomizer { server ->
