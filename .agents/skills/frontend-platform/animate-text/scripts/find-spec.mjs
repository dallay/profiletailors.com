...
function scoreSpec(spec, query, terms) {
  ...
  for (const term of terms) {
    ...
    if (usageNotes.includes(term)) {
      score += 8;
    }
    if (inspiration.includes(term)) {
      score += 6;
    }
    if (target.includes(term)) {
      score += 5;
    }
    if (renderer.includes(term)) {
      score += 4;
    }
  }
...
const visibleOrder = new Map(
    catalog.visible_ids.map((id, index) => [id, index]));
...
const results = readdirSync(fileURLToPath(specsDir))
    ...
    .map((fileName) => {
      const spec = readJson(
          new URL(`../assets/specs/${fileName}`, import.meta.url));
      const effect = readJson(
          new URL(`../assets/effects/${fileName}`, import.meta.url));
      return {...spec, effect};
    })
    .map((spec) => {
      return {
    ...
    .map(({spec, score}) => ({
      ...
      target: spec.target,
      renderer: spec.effect?.showcase?.renderer?.id ?? spec.custom_renderer
          ?? null,
      visible: spec.visibility ? spec.visibility === 'visible'
          : visibleOrder.has(spec.id),
      score,
    }));
