...
Add variables to the file at `tailwindCssFile` from `npx shadcn-vue@latest info` (typically
...
When `tailwindVersion` is `"v3"` (check via `npx shadcn-vue@latest info`), register in
