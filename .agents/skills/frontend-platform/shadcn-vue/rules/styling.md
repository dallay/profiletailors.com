...
ask the user about adding a custom CSS variable to the theme (
...
Use `class` for layout (e.g. `max-w-md`, `mx-auto`, `mt-4`), **not** for overriding component colors
or typography. To change colors, use semantic tokens, built-in variants, or CSS variables.
...
<template>
  <div
  :class="cn('flex items-center', isActive ? 'bg-primary text-primary-foreground' : 'bg-muted')">
</template>
```
