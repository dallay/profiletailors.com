...
test.describe('Bulk import @bulk @e2e', () => {
  ...
  test('upload csv preview validate schedule poll', async ({ page }) => {
    await page.route('**/api/v1/workspaces/**/bulk/validate', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          rows: [
            { rowIndex: 0, status: 'VALID', errors: [], bodyText: 'Hello', scheduledFor: '2099-06-15T10:00:00Z' },
            { rowIndex: 1, status: 'INVALID', errors: [{ code: 'INVALID_DATE', message: 'bad date' }] },
          ],
        }),
      })
    })
    await page.route('**/api/v1/workspaces/**/bulk/schedule', async (route) => {
      await route.fulfill({
        status: 207,
        contentType: 'application/vnd.api.v1+json',
        body: JSON.stringify({ jobId: 'job-1', totalRows: 2, scheduledCount: 1, failedCount: 1, rows: [] }),
      })
    })
    await page.route('**/api/v1/workspaces/**/bulk/jobs/**', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/vnd.api.v1+json',
        body: JSON.stringify({ jobId: 'job-1', status: 'PARTIAL', totalRows: 2, scheduledCount: 1, failedCount: 1, rows: [] }),
      })
    })
    await page.route('**/api/v1/workspaces/**/bulk/templates', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/vnd.api.v1+json',
        body: JSON.stringify({ templates: [{ id: 'linkedin-calendar', name: 'LinkedIn Calendar', description: 'default', header: 'bodyText,scheduledFor,timezone,media_urls,hashtags' }] }),
      })
    })
    await page.route('**/api/v1/workspaces/**/bulk/templates/**/csv', async (route) => {
      await route.fulfill({ status: 200, contentType: 'text/csv', body: 'bodyText,scheduledFor,timezone,media_urls,hashtags\n' })
    })

    await page.getByTestId('open-bulk-import').click()
    await expect(page.getByTestId('bulk-import-modal')).toBeVisible()

    const csvContent = 'bodyText,scheduledFor,timezone,media_urls,hashtags\nHello,2099-06-15T10:00:00Z,UTC,,\n,not-a-date,UTC,,'
    ...
    await page.getByTestId('bulk-validate-btn').click()
  ...
  test('CSV header validation shows error via real parser', async ({ page }) => {
    ...
    await expect(page.getByTestId('bulk-header-error')).toContainText('bodyText,scheduledFor,timezone,media_urls,hashtags')
  })
  ...
  test('template picker downloads CSV into textarea', async ({ page }) => {
    ...
    if (await downloadBtn.isVisible().catch(() => false)) {
      ...
      await page.getByTestId('bulk-csv-textarea').fill('bodyText,scheduledFor,timezone,media_urls,hashtags\nTemplate row,2099-06-20T10:00:00Z,UTC,,')
      await expect(page.getByTestId('bulk-csv-textarea')).toHaveValue(/Template row/)
    }
  })
})
