...
describe('useIdeaComposer', () => {
  ...
  it('initializes draft from idea for edit mode', () => {
    const idea = makeIdea({
      ...
      notes: 'hello **bold**',
      tags: ['Vue'],
      links: [{ url: 'https://vuejs.org', label: null }],
      columnId: 'done',
    })
    const composer = useIdeaComposer({
      idea,
      columns: ref(makeColumns()),
      store: { createIdea: vi.fn(), updateIdea: vi.fn(), saving: ref(false) },
    })
  ...
  it('blocks save when title empty and does not call store', async () => {
    ...
    const composer = useIdeaComposer({
      idea: null,
      columns: ref(makeColumns()),
      store: { createIdea, updateIdea: vi.fn(), saving: ref(false) },
    })
  ...
  it('duplicate-save guard prevents second call while saving', async () => {
    ...
    const composer = useIdeaComposer({
      idea: null,
      columns: ref(makeColumns()),
      store: { createIdea, updateIdea: vi.fn(), saving: storeSaving },
    })
  ...
  it('column change persists via save', async () => {
    ...
    const composer = useIdeaComposer({
      idea,
      columns: ref(makeColumns()),
      store: { createIdea: vi.fn(), updateIdea, saving: ref(false) },
    })
...
describe('buildPublishingPrefill', () => {
  it('joins title, notes and hashtags with dedupe', () => {
    const prefill = buildPublishingPrefill({
      title: 'Launch idea',
      notes: 'Focus on ROI',
      tags: ['launch', 'roi'],
    } as Idea)
  ...
  it('avoids duplicating tags already present as hashtags in notes', () => {
    const prefill = buildPublishingPrefill({
      title: 'T',
      notes: 'Notes about #kafka and streams',
      tags: ['kafka', 'testing'],
    } as Idea)
  ...
  it('handles empty notes and tags', () => {
    expect(
      buildPublishingPrefill({ title: 'Only title', notes: null, tags: [] } as unknown as Idea),
    ).toBe('Only title')
    expect(buildPublishingPrefill({ title: 'T', notes: 'N', tags: [] } as unknown as Idea)).toBe(
      'T\n\nN',
    )
    expect(
      buildPublishingPrefill({ title: 'T', notes: null, tags: ['vue'] } as unknown as Idea),
    ).toBe('T\n\n#vue')
  })
