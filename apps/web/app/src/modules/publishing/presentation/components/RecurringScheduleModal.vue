...
const weekDays = computed(() => [
  t('postDetail.recurring.days.monday'), t('postDetail.recurring.days.tuesday'), t('postDetail.recurring.days.wednesday'),
  t('postDetail.recurring.days.thursday'), t('postDetail.recurring.days.friday'), t('postDetail.recurring.days.saturday'), t('postDetail.recurring.days.sunday'),
])
...
function initialize(): void {
  ...
  daysOfWeek.value = schedule?.daysOfWeek?.length ? [...schedule.daysOfWeek] : [0]
...
async function save(): Promise<void> {
  ...
  if (frequency.value === 'weekly' && daysOfWeek.value.length === 0) {
    error.value = t('postDetail.recurring.errors.weekdayRequired')
    return
  }
  isSaving.value = true
  try {
    const input: RecurringScheduleInput = {
      ...
      dayOfMonth: frequency.value === 'monthly' ? Math.max(1, dayOfMonth.value || 1) : null,
    ...
    error.value = cause instanceof Error ? cause.message : t('postDetail.recurring.errors.saveFailed')
  } finally {
...
<template>
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 p-4">
      ...
      <dialog ref="container" open class="relative m-0 w-full max-w-xl rounded-2xl border border-border-subtle bg-bg-surface p-0 text-text-body shadow-2xl" aria-modal="true" aria-labelledby="recurring-title" @keydown.escape="emit('close')">
        <form class="space-y-5 p-6" @submit.prevent="save">
          <header>
            <h2 id="recurring-title" class="font-mono text-sm font-bold uppercase tracking-widest text-text-display">
              {{ isEditing ? t('postDetail.recurring.editTitle') : t('postDetail.recurring.title') }}
            </h2>
            <p class="mt-1 text-xs text-text-secondary">{{ t('postDetail.recurring.description') }}</p>
          </header>
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <label class="space-y-1 text-xs font-mono uppercase tracking-wider">
              {{ t('postDetail.recurring.interval') }}
          ...
          <fieldset v-if="frequency === 'weekly'" class="space-y-2">
            <legend class="text-xs font-mono uppercase tracking-wider">{{ t('postDetail.recurring.weekdays') }}</legend>
          ...
          <label class="block space-y-1 text-xs font-mono uppercase tracking-wider">
            {{ t('postDetail.recurring.maxOccurrences') }}
