...
This document describes the monitoring setup for the Profile Tailors Social Media Platform (SMP) server using Prometheus and Grafana. The setup provides real-time visibility into application health, performance metrics, and infrastructure status.
...
A pre-configured dashboard **"Spring Boot SMP - Overview"** is automatically provisioned and includes panels for all key metrics mentioned above.
