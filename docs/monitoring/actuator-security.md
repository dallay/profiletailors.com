...
Spring Boot Actuator endpoints often expose sensitive information about application internals, environment variables, and performance. This document outlines the security strategy implemented to protect these endpoints in the Profile Tailors monorepo.

## Security Strategy: Layered Access

We use a **Port Separation** strategy combined with **Spring Security** rules to ensure maximum safety.

### 1. Port Separation

In production environments, Actuator is configured to run on a dedicated management port (`9091`), while the public API remains on port `8080`.

-   **Port 8080**: Public traffic. Only the basic health status is exposed.
-   **Port 9091**: Internal traffic. Accessible only by Prometheus and operators within the internal network (VPC/VPN).
...
### Docker Networks
Services are isolated using Docker networks. Prometheus and SMP share the `monitoring` network where port `9091` is reachable.
