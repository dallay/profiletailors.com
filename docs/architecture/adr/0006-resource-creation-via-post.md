...
There was an earlier architectural consideration for `PUT`-based resource creation (with
client-generated IDs) to support offline-first scenarios.
...
New resources MUST be created using `POST` endpoints where the server is responsible for generating
the unique identifier.
Idempotency for creation SHOULD be handled via specific logic (e.g., uniqueness constraints on
natural keys) rather than relying on client-supplied UUIDs in a `PUT` path.
...
- Reason deferred: Offline support is currently a non-goal. The overhead of managing
  client-generated ID collisions and logic is not justified at this stage.
