...
## Context
Standard Service-oriented architectures often lead to bloated service classes that violate the Single Responsibility Principle.
...
A "relaxed" CQRS approach is permitted where Commands MAY return data (using `CommandWithResult`) to simplify frontend integration and reduce round-trips.
