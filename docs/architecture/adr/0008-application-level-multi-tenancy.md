...
- **Resolution**: The backend MUST extract this header and store it in the `ResourceContext` for the
  duration of the request.
- **Filtering**: Every database query for tenant-scoped data MUST include a
  `WHERE workspace_id = :workspaceId` clause.

## Scope and boundaries

- All repositories under `com.profiletailors.smp`.
- Cross-tenant lookups (e.g., admin views) MUST be explicitly authorized and bypass standard
  filters.
...
- Disadvantages: More complex to manage migrations, harder to debug, and requires consistent setting
  of session variables.
- Reason rejected: Application-level filtering is sufficient for the current scale and easier for
  developers to implement with R2DBC.
...
- Potential performance impact on very large tables if indexes aren't correctly applied to
  `workspace_id`.
