...
**Created:** `server/smp/src/main/kotlin/com/profiletailors/smp/platform/infrastructure/http/WebFluxConfiguration.kt`
...
Spring Boot 4's built-in resolvers don't support vendor media type patterns like `application/vnd.api.v{version}+json`. We need a custom resolver to:
