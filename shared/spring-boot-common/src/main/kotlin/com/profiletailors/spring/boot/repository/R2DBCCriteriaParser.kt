...
import com.profiletailors.common.domain.criteria.CriteriaParser
...
class R2DBCCriteriaParser<T : Any>(clazz: KClass<T>) : CriteriaParser<R2DBCCriteria> {
    private val columnNames = clazz.memberProperties.associate { it.name to columnName(it) }

    @Suppress("CyclomaticComplexMethod", "CognitiveComplexMethod")
    override fun parse(criteria: Criteria): R2DBCCriteria = when (criteria) {
    ...
    private fun parse(criteria: Criteria.And): R2DBCCriteria {
        if (criteria.value.isEmpty()) return R2DBCCriteria.empty()
        if (criteria.value.size == 1) return parse(criteria.value[0])
    ...
    private fun parse(criteria: Criteria.LessThan): R2DBCCriteria = where(criteria.key).lessThan(criteria.value)

    private fun parse(criteria: Criteria.LessThanEquals): R2DBCCriteria =
        where(criteria.key).lessThanOrEquals(criteria.value)

    private fun parse(criteria: Criteria.GreaterThan): R2DBCCriteria = where(criteria.key).greaterThan(criteria.value)
    ...
    private fun parse(criteria: Criteria.Like): R2DBCCriteria = where(criteria.key).like(criteria.value)

    private fun parse(criteria: Criteria.Ilike): R2DBCCriteria =
        where(criteria.key).like(criteria.value).ignoreCase(true)

    private fun parse(criteria: Criteria.NotLike): R2DBCCriteria = where(criteria.key).notLike(criteria.value)

    private fun parse(criteria: Criteria.In): R2DBCCriteria = where(criteria.key).`in`(criteria.value)

    private fun parse(criteria: Criteria.NotIn): R2DBCCriteria = where(criteria.key).notIn(criteria.value)
    ...
    private fun where(key: String): R2DBCCriteriaStep = R2DBCCriteria.where(
