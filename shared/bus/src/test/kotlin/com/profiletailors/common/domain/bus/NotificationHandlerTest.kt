...
class NotificationHandlerTest {
    ...
    fun inherited_notification_handler_should_be_called() = runTest {
        ...
        val handlers: HashMap<Class<*>, Any> =
            hashMapOf(Pair(InheritedNotificationHandler::class.java, nHandler))
        val provider = ManualDependencyProvider(handlers)
        val bus: Mediator = MediatorBuilder(provider).build()
    ...
    inner class ParamaterizedTests {
        ...
        inner class ParameterizedNotificationHandler<A> :
            NotificationHandler<ParameterizedNotification<A>> {
        ...
        fun inherited_notification_handler_should_be_called() = runTest {
            ...
            class ParameterizedNotificationHandler<A> :
                NotificationHandlerBase<ParameterizedNotification<A>>() {
                override suspend fun handle(notification: ParameterizedNotification<A>) {
            ...
            val handlers: HashMap<Class<*>, Any> =
                hashMapOf(Pair(ParameterizedNotificationHandler::class.java, nHandler))
            val provider = ManualDependencyProvider(handlers)
            val bus: Mediator = MediatorBuilder(provider).build()
