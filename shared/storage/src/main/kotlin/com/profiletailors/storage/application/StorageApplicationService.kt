...
class StorageApplicationService(
    ...
    private val provider: String = StorageMetrics.Providers.LOCAL
) {
    ...
    suspend fun upload(
        ...
        try {
            ...
            metrics.recordOperation(StorageMetrics.Operations.UPLOAD, provider, bucket, true)
            onUploadSuccess(bucket, key, totalSize, uploaderId, metadata)
        } catch (e: Exception) {
            val errorType = when (e) {
                is StorageSecurityException -> StorageMetrics.ErrorTypes.SECURITY
                is StorageObjectNotFoundException -> StorageMetrics.ErrorTypes.NOT_FOUND
                else -> StorageMetrics.ErrorTypes.SERVICE
            }
            metrics.recordError(StorageMetrics.Operations.UPLOAD, provider, bucket, errorType)
            metrics.recordOperation(StorageMetrics.Operations.UPLOAD, provider, bucket, false)
            throw e
        }
    }
    ...
    fun download(
        ...
        return channelFlow {
            ...
            try {
                ...
                metrics.recordOperation(StorageMetrics.Operations.DOWNLOAD, provider, bucket, true)
            } catch (e: Exception) {
                val errorType = when (e) {
                    is StorageSecurityException -> StorageMetrics.ErrorTypes.SECURITY
                    is StorageObjectNotFoundException -> StorageMetrics.ErrorTypes.NOT_FOUND
                ...
                metrics.recordError(StorageMetrics.Operations.DOWNLOAD, provider, bucket, errorType)
                metrics.recordOperation(StorageMetrics.Operations.DOWNLOAD, provider, bucket, false)
                throw e
            }
        }
    }
    ...
    suspend fun delete(
        ...
        try {
            metrics.recordOperationTime(StorageMetrics.Operations.DELETE, provider) {
                storage.delete(bucket, key)
            }
            metrics.recordOperation(StorageMetrics.Operations.DELETE, provider, bucket, true)
            ...
            metrics.recordError(StorageMetrics.Operations.DELETE, provider, bucket, errorType)
            metrics.recordOperation(StorageMetrics.Operations.DELETE, provider, bucket, false)
            throw e
        }
    }
    ...
    suspend fun list(bucket: String, prefix: String = ""): List<String> =
        metrics.recordOperationTime(StorageMetrics.Operations.LIST, provider) {
            storage.list(bucket, prefix)
        }.also {
            metrics.recordOperation(StorageMetrics.Operations.LIST, provider, bucket, true)
        }
