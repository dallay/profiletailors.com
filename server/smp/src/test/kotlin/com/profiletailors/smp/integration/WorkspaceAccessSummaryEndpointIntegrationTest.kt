...
class WorkspaceAccessSummaryEndpointIntegrationTest(
    ...
    @Test
    fun `workspace access summary remains unaffected by target scopes on h2`() {
        seedAuthorizedMember(entitled = true)
    ...
    @Test
    fun `direct deny overrides entitled role based allow on h2`() {
        seedAuthorizedMember(entitled = true)
        seedDirectGrant(
            effect = "DENY",
    ...
    @Test
    fun `expired direct grant is ignored on entitled h2 member`() {
        seedMemberWithoutPermission(entitled = true)
        seedDirectGrant(
            effect = "ALLOW",
            permissionId = "permission-1",
    ...
    @Test
    fun `denies authorized principal when workspace entitlement is missing on h2`() {
    ...
    @Test
    fun `allows old api key before replacement and new api key after replacement on h2`() {
        seedAuthorizedApiKeyPrincipal(entitled = true)
