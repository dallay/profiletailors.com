...
class SocialContentCalendarCursorCodecTest {
    ...
    @Test
    fun `trims surrounding whitespace before decoding`() {
        val padded = "  ${SocialContentCalendarCursorCodec.encode(cursor)}\n\t"

        SocialContentCalendarCursorCodec.decode(padded) shouldBe cursor
    }

    @Test
    fun `rejects tokens containing standard base64 characters`() {
        val nonUrlSafeToken = "+${SocialContentCalendarCursorCodec.encode(cursor)}"

        shouldThrow<InvalidSocialContentCursorException> {
            SocialContentCalendarCursorCodec.decode(nonUrlSafeToken)
        }
    }
