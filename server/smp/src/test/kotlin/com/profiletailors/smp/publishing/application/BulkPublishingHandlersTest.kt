...
class BulkPublishingHandlersTest {
    private val validationPipeline = BulkValidationPipeline(ProviderCapabilityValidator { }, Clock.fixed(Instant.parse("2026-01-01T00:00:00Z"), ZoneId.of("UTC")))
    ...
    private val scheduleHandler = ScheduleBulkHandler(
        principalContextProvider, resourceContextProvider, validationPipeline, bulkImportJobRepository, publicationCreationService, transactionRunner, clock, socialAccountRepository
    )
    ...
    fun `schedule handler chunked 50-100 with 200_207 partial`() = runTest {
        ...
        val principalId = "u-1"
        coEvery { principalContextProvider.require() } returns PrincipalContext(principalId, PrincipalType.USER, principalId)
        ...
        coEvery { socialAccountRepository.findFirstActiveByWorkspace(workspaceId) } returns SocialAccount(
            id = "acc-1", socialConnectionId = "conn-1", workspaceId = workspaceId, provider = SocialProvider.LINKEDIN,
            providerAccountId = "p-1", kind = SocialAccountKind.PERSONAL_PROFILE, displayName = "Active", status = SocialConnectionStatus.ACTIVE,
        )
        coEvery { publicationCreationService.create(any(), any(), any(), any(), any(), any(), any(), any(), any()) } answers {
        ...
        val result = scheduleHandler.handle(ScheduleBulkCommand(workspaceId = workspaceId, csvText = csv, csvHash = csv))
    ...
    fun `schedule handler throws 409 on duplicate sha256`() = runTest {
        ...
        val existingJob = BulkImportJob(
            id = "bulk-existing", workspaceId = workspaceId, principalId = principalId,
            idempotencyKey = BulkImportJob.computeIdempotencyKey(workspaceId, principalId, "hash"),
            csvHash = "hash", status = BulkJobStatus.SCHEDULED, totalRows = 1, createdAt = Instant.now(),
        )
