        ...
        return spec.map { row, _ ->
            RecurringSchedule(
                ...
                recurrenceRule = RecurrenceRule(
                    ...
                    interval = requireNotNull(row.get("recurrence_interval", Integer::class.java)).toInt(),
                    daysOfWeek = row.get("days_of_week", String::class.java).orEmpty().split(",").filter { it.isNotBlank() }.map { it.toInt() }.toSet(),
                    dayOfMonth = row.get("day_of_month", Integer::class.java)?.toInt(),
                    endDate = row.get("end_date", LocalDate::class.java),
                    maxOccurrences = row.get("max_occurrences", Integer::class.java)?.toInt(),
                ),
