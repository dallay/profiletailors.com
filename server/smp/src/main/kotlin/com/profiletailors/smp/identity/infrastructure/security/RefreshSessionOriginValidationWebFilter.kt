...
class RefreshSessionOriginValidationWebFilter(
    ...
    private fun requiresOriginValidation(method: HttpMethod?, path: String, headers: HttpHeaders): Boolean {
        ...
        return headers.getOrEmpty(HttpHeaders.COOKIE).any { cookie ->
            cookie.contains("${refreshSessionProperties.cookieName}=", ignoreCase = false)
        }
    }

    private fun extractSourceOrigin(headers: HttpHeaders): String? {
        val origin = headers.getFirst(HttpHeaders.ORIGIN)
            ?.takeIf(String::isNotBlank)
            ?.let(::normalizeOrigin)
        if (origin != null) return origin

        return parseRefererOrigin(headers.getFirst(HttpHeaders.REFERER))
    }

    @Suppress("ReturnCount")
    private fun parseRefererOrigin(referer: String?): String? {
        if (referer.isNullOrBlank()) return null
        val uri = runCatching { URI.create(referer) }.getOrNull() ?: return null
