...
interface UnsplashPhotoProvider {
    /** Lists editorial photos when [query] is blank and searches when it is present. */
    suspend fun search(query: String?): List<UnsplashPhoto>

    /** Resolves the current canonical provider data for one photo. */
    suspend fun get(externalId: String): UnsplashPhoto

    /** Streams the selected photo bytes from the provider CDN. */
