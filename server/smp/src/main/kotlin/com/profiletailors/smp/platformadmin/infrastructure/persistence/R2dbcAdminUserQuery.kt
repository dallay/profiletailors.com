...
class R2dbcAdminUserQuery(private val databaseClient: DatabaseClient) : AdminUserQuery {
    ...
    override suspend fun list(query: ListAdminUsersQuery): PagedResult<AdminUserSummary> {
        ...
        val dataSql = """
            SELECT p.id, p.principal_type, p.created_at, p.display_identity, p.status, p.version,
                   ui.email
            FROM principals p
    ...
    companion object {
        ...
        private const val SELECT_USER_DETAIL = """
            SELECT p.id, p.principal_type, p.created_at, p.display_identity, p.status, p.version, ui.email
