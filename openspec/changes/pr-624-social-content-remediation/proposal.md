...
**New Capabilities**: None - this is remediation of the PR #624 social-content foundation.

**Modified Capabilities**: publishing - typed capability denial, bounded/resumable social-content synchronization, deterministic reply idempotency, domain invariants, and workspace-safe social-content persistence.
...
handlers.importPosts(actor, now)
```

## Troubleshooting

- Handler extraction: If tests fail, verify ports remain unchanged and the façade delegates correctly.
- Cursor guards: If pagination loops, check repeated-cursor detection and max-page enforcement.
- FK migration: If rollback fails, verify existing data satisfies composite workspace constraints.
- Review replies: If comments seem stale, check design.md and apply-progress.md for evidence classification.
...
- Dependencies: Existing Kotlin/Gradle, JUnit/Kotest, Cucumber, R2DBC/Testcontainers, Liquibase, and PR #624 review context

### Success Criteria

- [x] Every in-scope valid finding has a tested fix or an explicit documented decision.
- [x] Posts/comments are bounded, repeated cursors are detected, and checkpoints have explicit safe semantics.
- [x] Reply behavior for SUCCEEDED/FAILED/PROCESSING is deterministic and tested.
- [x] Focused backend tests and selected integration checks pass without application changes outside scope.
- [ ] Required replies classify architecture-docs-sync.md, ratelimit, and mutationAllowed comments with evidence (evidence recorded, GitHub replies pending).
