...
## Addendum 2026-09-08 — CRITICAL-2 fixed + CodeRabbit review round (PR #988)

CRITICAL-2 (counters) fixed via `InvitationTelemetry` port + `InvitationObservability` Micrometer adapter, wired in create/revoke handlers and asserted with mockk `verify`. WARNING-7 (9 introduced Detekt findings) fixed: commands moved to `command/` package, `LongMethod`/`ThrowsCount` refactored, test literals cleaned; `:server:smp:detekt` clean. New BDD glue split (`DirectInvitationBddSteps` + `PlatformAdminScenarioState`) resolved the pre-existing LargeClass gate failure without suppressions. Architecture violations from the new event resolved (dead `InvitationRevoked` deleted, `DirectInvitationResent` carries `@NamedInterface("domain")` mirroring `InvitationIssued`, rule allowlist extended); `HexagonalArchTest` + `ModularStructureTest` green. 401 direct scenario added. Merge with `origin/main` resolved (562 archive taken from main's canonical completed archive; `HexagonalArchTest` unioned with main's new observability/data-access rules).

CodeRabbit round (38 threads on PR #988) — applied: email trim+lowercase normalization; DIRECT-only resend guard; `isActive(now)` guards on revoke AND resend (no revival of expired rows); `version` in create/resend results with resend→revoke BDD coverage; unique-conflict → `InvitationAlreadyActiveException` mapping; expiry-aware dedup query; SPA resend action + stored recipient email + returned-version revoke + versioned `Content-Type` + typed Vitest mock; `TooManyFunctions` suppression removed; 568 proposal/design/specs/tasks aligned to implemented routes/events/contracts; 563 tasks verification record corrected.

Deliberately NOT applied (verified against repo evidence): T14 (remove `@NamedInterface`) — `InvitationIssued.kt:22` carries the same annotation with the same name-exclusion, the change mirrors precedent; T35 (repository out of `ResendInvitationHandler`) — `ResendWaitlistInvitationHandler` injects its repository port directly, same pattern; T27 (Overview→References structure on delta specs) — that structure governs prose docs, delta specs use requirement/scenario format; T18/T37/T38 (Kotest + `should`-naming) — same-package neighbor `ResendWaitlistInvitationHandlerTest` uses AssertJ/JUnit with backtick `throws/when` style, new tests match context. Threads 4–9/20 (stale 562 archive copies) mooted by taking main's canonical archive.

Deferred: task 4.8 (workspace-existence validation at creation — no workspace read port in `platformadmin`).

## Verdict (current)

**PASS WITH WARNINGS** — all MUST-level items fixed and gated (unit 146/146 platformadmin, BDD direct 11/11 incl. resend→revoke-with-version, Detekt 0, admin type-check/lint/Vitest 29/29, `just ci` 15/15 on the pre-round base; full re-run pending on this round). Warnings: task 4.8 deferred; skipped threads above with recorded rationale.
