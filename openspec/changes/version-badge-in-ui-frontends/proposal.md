...
The badge shows the deployed version from each frontend's `package.json` and the short git SHA of the build. Hovering the badge exposes the build timestamp.

The landing page is excluded because it is the unauthenticated marketing surface and intentionally omits implementation details.

## Troubleshooting

If the displayed build information is unexpected, verify the frontend's build-time constants and the `GIT_SHA` override used for that build. The badge does not query a runtime or backend version endpoint.
