...
- [ ] 2.2 GREEN: create `scripts/extract-release-info.mjs` exporting `stripScopeFromTag`, `shortSha`, `resolveReleaseSha`, and `extractReleaseInfo`; `scripts/extract-release-info.test.mjs` passes with `node --test`.
...
- [ ] 2.4 Add the six-step body to each deploy job: SHA-pinned `actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1 # v7.0.1` (`ref: <tag>`, `fetch-depth: 0`, `persist-credentials: false`); `id: resolve-meta` running `node scripts/extract-release-info.mjs` with `--tag`, `--scope`, `--provided` args; `.github/actions/setup-frontend`; `pnpm --filter <workspace> build` with `env.GIT_SHA` from `${{ steps.resolve-meta.outputs.git_sha }}`; SHA-pinned `cloudflare/wrangler-action@9acf94ace14e7dc412b076f2c5c20b8ce93c79cd # v3.15.0` invoking `pages deploy <dist> --project-name=<canonical> --branch=production --commit-dirty=true`; verification step.
