...
- [x] 1.1 Ports + DTO in `governance.application` (`AdminTakedownQueryPort` / `CommandPort`,
  `AdminTakedownPage`, status `String`, `assetStatus` detail-only)
...
- [x] 1.6 RED adapter tests (approve/reject lifecycle, not-found, not-reviewable, operator id, no
  `ResourceContext`)
- [x] 1.7 GREEN query/command adapters using domain `approve`/`dismiss`, updater, `AuditHook`,
  emails
- [x] 1.8 HexagonalArchTest: ports in `governance.application`; `TakedownReport` stays inside
  governance
...
Untouched: workspace `ApproveTakedownHandler` / `RejectTakedownHandler`, platformadmin, frontend,
BDD.
...
- `just backend-test-fast` unfiltered (user asked focused checks; architecture owners run
  explicitly)
...
- Budget concern: over 400 including tests. Production slice is near budget. Tests stay with the
  work unit.
...
None from design except `AdminTakedownPage.from` instead of `of` to avoid a new Detekt
`FunctionNameMinLength` baseline entry (`PagedResult.of` is already baselined).
...
- `JacksonConfigurationIdempotencyCodec` owns a private `ObjectMapper` with `JavaTimeModule`
  registered. Rationale: Spring injects the shared Jackson-2 `ObjectMapper` bean (Kotlin module
  only)
  into the codec constructor, ignoring the Kotlin default argument — so the constructor default
  never
...
- New Liquibase changeset `011-widen-configuration-idempotency-command.yaml` widens `command`
  32 → 128 (`approve_takedown:<reportId>` needs ~66 chars). `010` left untouched (immutable
  history).
...
- Frontend: `GovernanceView.vue` (list/filter/detail/approve/reject, manage-gated,
  `Idempotency-Key`,
