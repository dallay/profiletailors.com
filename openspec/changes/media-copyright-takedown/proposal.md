...
Profile Tailors imports Unsplash images without visible attribution — violating Unsplash license terms (attribution required for both platform and photographer). The platform also has no DMCA takedown mechanism, creating legal exposure. This change adds attribution metadata display and a complete takedown lifecycle.
...
- `media-takedown`: Report submission → review → suspension → counter-notice → restore, with email + audit

### Modified Capabilities
- `media-library`: Extended DTOs with `licence` field; `SUSPENDED` status in lifecycle
- `email-notifications`: Takedown email templates (submission, suspension, counter-notice, restoration)
...
3. REST: `POST /api/governance/media/takedown-reports`, `GET` (list), `POST .../{id}/action` (review)
...
- **Phase 2**: Disable takedown endpoints via feature flag. Bulk-update `SUSPENDED` → `READY`. Liquibase rollback removes new tables.
