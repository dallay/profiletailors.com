...
- WHEN calling `POST .../action` on a takedown report
- THEN the `WorkspaceAuthorizationDecider` SHALL deny access
...
- WHEN calling `POST .../action` on a takedown report
- THEN the `WorkspaceAuthorizationDecider` SHALL grant access
