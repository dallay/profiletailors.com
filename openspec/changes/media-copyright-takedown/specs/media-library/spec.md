...
in `SUSPENDED` state are not retryable and SHALL NOT transition to `PROCESSING` without a
counter-notice approval. The `FAILED → PROCESSING` retry path SHALL NOT apply to `SUSPENDED` assets.
