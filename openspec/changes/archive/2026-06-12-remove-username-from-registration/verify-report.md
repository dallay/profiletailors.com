...
- Consider adding an explicit test that verifies the server tolerates (ignores) unknown `username`
  field in the register request body. The spec allows both 201 and 400, and Jackson's current
  behavior is 201, but a regression could make this a breaking change.
...
All 10 tasks complete. All 7 spec scenarios compliant. Backend build passes (51s). Frontend tests
pass (47/47). No dead i18n keys. Design decisions followed exactly. No regressions detected.
