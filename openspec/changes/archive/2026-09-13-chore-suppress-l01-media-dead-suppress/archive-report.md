...
- proposal.md ✅ (aligned with the amended 1-deletion scope; `StaleAssetReconciler.kt` explicitly
  deferred)
...
- `server/smp/.../media/application/MediaHandlers.kt`: exactly 1 deleted line
  (`@Suppress("TooGenericExceptionCaught")`), zero added
...
- `StaleAssetReconciler.kt:96` annotation retained byte-identical; any potential removal requires a
  separately scoped change with tool-run `detekt-baseline.xml` regeneration (never hand-edit `:88`)
