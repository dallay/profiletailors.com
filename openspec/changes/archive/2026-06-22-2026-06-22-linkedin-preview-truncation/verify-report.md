...
1. **`pnpm type-check` in `apps/web/app` now passes.**
    ...
    - The TypeScript errors in `CreatePostModal.vue` and `CreatePostModal.test.ts` that blocked the
      previous gate have been resolved.

### INFO (no action required)

1. Task **3.4** wording references `just frontend-test` which runs the Astro marketing app suite,
   not the Vue app suite.
...
2. `PostPreviewPanel.vue` establishes the provider seam; only the LinkedIn branch is implemented.
    - This is by design for the current slice; future providers can extend via the typed enum
      without modifying LinkedIn rules.

## Summary

All spec scenarios are covered by passing runtime tests (28 targeted, 449 total). The
`pnpm type-check` gate that blocked the previous pass is now clear. All proposal success criteria,
spec requirements, design decisions, and tasks are satisfied. The implementation is compliant and
