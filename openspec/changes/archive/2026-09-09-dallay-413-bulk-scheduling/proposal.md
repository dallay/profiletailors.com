...
Bulk CSV scheduling. Publishing is single-post only; need validated preview + chunked atomic
schedule without duplicating lifecycle rules.
...
- Reuse `PublicationLifecyclePolicy`, `ProviderCapabilityValidator`, `ConflictDetectionPolicy`,
  `MediaAssetResolver`
- Frontend: `useBulkImport`/`useBulkCsvParser`, `BulkImportModal`, `BulkPreviewTable`,
  `BulkTemplatePicker`
...
- `publishing`: bulk validate/schedule/job/template, `BulkImportJob` lifecycle, `media_urls` via
  `EXTERNAL_URL`

## Approach

Sync chunked batch inside publishing (Approach 1). `BulkPublishingController` at
`/api/v1/.../bulk/*` reuses `RecurringScheduleController` guard. Extract
`PublicationCreationService`; schedule chunks per transaction. Frontend CSV → validate → fix →
schedule → poll.
...
Flag `bulkScheduling.enabled` off. Rollback `022`/`021`. Remove routes. Existing jobs remain;
cleanup via `bulk_import_rows` join. Frontend flag-guarded.
