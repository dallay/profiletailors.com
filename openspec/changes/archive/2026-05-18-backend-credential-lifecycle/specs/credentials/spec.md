...
(Previously: The implemented API-key path supported active/inactive/revoked validation on the
proving slice while API-key issuance, rotation workflows, inventory surfaces, broad metadata
expansion, and generalized credential-platform redesign remained deferred.)
