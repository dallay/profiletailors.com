...
For the existing `/api/authorization/workspace-access/current` proving slice, the same protection principles MUST apply to authenticated USER, authenticated SERVICE_ACCOUNT, and authenticated API_KEY principals.
...
(Previously: This change proved API-key allow, authorization-controlled deny, and revoked-or-inactive-credential deny outcomes, while rotation workflows, inventory surfaces, and broad credential-management redesign remained deferred.)
