...
(Previously: registration was unavailable on failure, without specifying in-route state, request suppression, or delayed entry-point rendering.)
...
When registration is disabled, the backend MUST reject `POST /api/auth/register` before command dispatch or mutation using the existing authoritative configuration. The response MUST be exact HTTP 503 Problem Details with `code: "REGISTRATION_DISABLED"`. No second registration flag or changed enabled-registration protocol MAY be introduced.
