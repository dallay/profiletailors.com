...
`App.vue` MUST render standalone route content without the authenticated shell for named routes
`login`, `register`, `forgot-password`, `reset-password`, and `verify-email`. All other routes MUST
retain the authenticated shell. Reset-password MUST be classified standalone without requiring a
guest session.

(Previously: only login/register rendered `AuthView` outside the shell; all other routes rendered
`AppShell`.)
