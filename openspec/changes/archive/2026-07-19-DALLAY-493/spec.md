...
Define the Data Subject Access Request (DSAR) aggregate, lifecycle state machine, authorization model, rate limits, and audit requirements for ACCESS, EXPORT, CORRECTION, and DELETION requests under GDPR/CCPA.

### Requirements

#### Requirement: DSAR Request Types and State Machine

The system MUST support four DSAR types: `ACCESS`, `EXPORT`, `CORRECTION`, `DELETION`. Every request MUST follow the state machine: `PENDING → IN_REVIEW → COMPLETED | REJECTED | FAILED`. Transitions from `COMPLETED`, `REJECTED`, or `FAILED` MUST be terminal. A `REJECTED` request MUST carry a `rejection_reason`.
...
Define how user data is collected across all bounded contexts for ACCESS/EXPORT requests, the response format, and delivery mechanism.
...
The `user_identities` table MUST support anonymization of `email` and `username` fields to `[REDACTED on {timestamp}]`. The `principals.display_identity` field MUST be set to `[REDACTED]` on deletion. These operations MUST be idempotent: calling anonymize on an already-anonymized record MUST NOT fail.
(Previously: PII fields are mutable by direct database operation only — no programmatic anonymization or correction endpoint exists.)
...
The identity context MUST expose a `CorrectUserIdentityCommand` handler supporting correction of `email` and `username` fields. Validation rules: `email` MUST be a valid email format, `username` MUST be 3–50 alphanumeric characters. The handler MUST return the previous values for rollback purposes.
...
`WaitlistEntryRepository` MUST support lookup of entries by normalized email (case-insensitive, trimmed). An `anonymizeEmail(entryId)` operation MUST replace the entry's `email` field with `[REDACTED on {timestamp}]` and clear any PII in `metadata` (set to empty JSON object `{}`).
...
When `email` is corrected on a `user_identities` row, the correction MUST propagate to all waitlist entries that match both the old and new email. The handler MUST NOT modify waitlist entries that match only the old email or only the new email — only entries matching BOTH are affected (e.g., a waitlist entry with the old email is updated to the new email).
...
Before any deletion phase, the system MUST validate: (1) the principal is not the sole owner of any workspace (`WorkspaceOwnershipPolicy`), (2) no publications are in `PENDING`/`PUBLISHING` state, (3) no outstanding contractual obligations (active subscriptions). If sole owner, the system MUST reject with `sole_workspace_owner` and a message directing to transfer ownership first.
