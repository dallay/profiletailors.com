...
is unchanged. Source-side evidence is in
`git diff -- server/smp/src/test/kotlin/com/profiletailors/smp/media/application/FakeStorageApplicationService.kt`.
...
be detected. All suites that previously crashed with
`wrong name: com/profiletailors/smp/media/application/NoopEventPublisher`
now exit 0.
...
1. **Source**: the dead public `class NoOpEventPublisher` and the dead
   `testStorageApplicationService`
