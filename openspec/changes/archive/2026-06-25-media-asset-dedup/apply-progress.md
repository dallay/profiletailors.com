...
`media-asset-dedup` (authoritative spec: `openspec/changes/media-asset-dedup/spec.md` revision
v3.2).
...
- `server/smp/src/main/kotlin/com/profiletailors/smp/media/application/MediaRepositories.kt`
    - `markAsReadyFromDedup(...)` now accepts optional `fileSizeBytes` so the upload handler can
      persist the streamed byte count.
...
- `server/smp/src/main/kotlin/com/profiletailors/smp/media/application/MediaHandlers.kt`
    ...
    - `CasUploadAssetHandler`:
        ...
        - Reordered the byte-count/hash finalization to compute `effectiveMediaType` after the
          upload
...
- `server/smp/src/main/resources/db/changelog/media/002-add-workspace-file-blobs.yaml`
    ...
    - Added `media-002-blob-ready-metadata-checks` enforcing READY metadata on
...
- The GC concurrent-worker scenario is covered structurally (the query uses `FOR UPDATE SKIP
  LOCKED`) but the apply phase did not run a multi-worker integration race; it remains a
  verify-phase
  improvement.
...
- `markAsReadyFromDedup` retains the legacy signature
  `(assetId, workspaceId, storageKey, detectedMediaType)`
...
`MediaCasHandlersTest` private class, which carries the FQ name
`com.profiletailors.smp.media.application.NoopEventPublisher`
(lowercase `oop`). The class loader then tries to resolve
`com.profiletailors.smp.media.application.NoOpEventPublisher`
...
`:server:smp:build`). Targeted
`--tests 'com.profiletailors.smp.media.application.MediaCasHandlersTest'`
...
anonymous `object : Storage {` and `object : PresignableStorage {` patterns, plus
`class ... : Storage`
...
frontend tests all now run clean with the override in place. The pre-existing
...
spec gap. The spec v3.2 contract for CAS upload finalize is unchanged; the production
implementations
