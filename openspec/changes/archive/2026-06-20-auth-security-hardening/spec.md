...
Allow users with unverified emails (PENDING) to authenticate and create sessions, while establishing a feature-level email verification policy for protected operations. Also introduce design contracts for future Argon2id password hashing migration.
...
Registration payload SHALL no longer return only RegistrationResult (breaking change from prior behavior).
