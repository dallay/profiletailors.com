...
- Database migration: add `email_status` column to `user_identities`, create `email_verification_tokens` table
...
- `email-verification`: Email verification lifecycle — token generation, email dispatch, token consumption, email status tracking
- `email-notifications`: Email sending infrastructure via SMTP — `SmtpEmailSender` adapter, Spring Boot Starter Mail integration

### Modified Capabilities

- `identity`: Registration flow now emits `UserRegistered` domain event instead of immediately issuing auth session; login/refresh check `email_status`
...
5. If migration already deployed with existing unverified users: manual SQL update to set all `email_status = 'VERIFIED'` before rollback
