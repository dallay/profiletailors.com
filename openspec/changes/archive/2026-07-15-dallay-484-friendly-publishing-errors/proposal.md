...
Frontend becomes the safety boundary: `PostDetailModal.vue` must never render raw unknown codes, `blockedReason`, or raw `Error.message`; all `FAILED`, `BLOCKED`, and action errors use i18n keys in `postDetail` EN/ES modules. Known failure categories get specific copy; missing, unknown, or historical raw values get a generic localized fallback.

Backend must map typed async failure signals to the canonical product taxonomy before persisting publication state. Media resolution, provider validation, and provider dispatch must run inside the worker failure boundary. Raw exception messages must not be stored on publications or notification events. Delivery attempts and logs retain only sanitized diagnostics suitable for server-side support.
...
- [ ] Media-resolution, validation, and provider-dispatch failures share the same guarded retry/failure path.
