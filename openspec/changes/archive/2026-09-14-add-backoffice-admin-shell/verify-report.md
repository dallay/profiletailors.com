...
Acceptance mapping (4 criteria): 1 PASS, 2 PASS, 3 PASS, 4 PASS
(criterion 4 via mirror tests in `auth.store.test.ts` + clean `admin-check`/`admin-build`).
...
1. **Delta spec text is factually wrong and must be amended before archive.**
   ...
   and "`platform.invitations.create` does not exist". Server truth
   (`PlatformPermission.kt` lines 6–21, `CreateInvitationHandler.kt` lines 42–43):
...
2. **Main spec role-permission mapping table lacks the `platform.invitations.create` row**
   (13 rows for 14 keys). Server grants it to OWNER + OPERATOR
   (`PlatformPermission.kt` lines 31–47). Required amendment: add the row
   (`✓ | ✓ | — | —`) in the same edit as (1).
