...
Coverage reports were generated. The configured threshold is `0`; no additional numeric coverage
gate applies.
...
- Vite warns that the main production chunk exceeds 500 kB after minification; this predates and is
  not caused by the catalog behavior.
...
**PASS WITH WARNINGS** — the corrective BDD merge closes the prior critical gap. Forced BDD
execution proves every required catalog, capacity/existing-channel, workspace-isolation,
OAuth-policy, authentication, and workspace-context scenario; backend checks, coverage generation,
SPA tests, and the SPA production build all pass. The remaining warnings are unrelated test/build
output only.
