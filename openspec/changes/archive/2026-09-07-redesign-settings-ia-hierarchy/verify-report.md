...
- Focused settings specs (5 files): **28/28 PASS** — `pnpm --filter app exec vitest run` on hero,
  locale, preferences-card, SettingsView, validation specs.
- `pnpm --filter app type-check` (`vue-tsc --build`): **PASS**, no errors.
- Scoped biome check on 7 touched files: **PASS**, no findings.
- Full build: **NOT RUN** — judged unnecessary; change is template/locale/test-only and is covered
  by type-check plus targeted vitest.
...
No logic changes; `segmentedControlClass` untouched; stores/router/services untouched; Preferences
card contains language only (no theme toggle moved).
...
**PASS** — all "What changes" items implemented, all "What does NOT change" items hold, gates green
in this run. Hand off to `sdd-qa` for acceptance.
