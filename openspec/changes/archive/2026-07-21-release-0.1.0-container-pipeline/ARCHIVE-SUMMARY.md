...
Implemented a complete automated container release pipeline that builds, pushes, and smoke-tests production images when release-please creates a release tag:
...
1. **`.github/workflows/release-image.yml`** — New tag-triggered workflow with three jobs:
   ...
   - `build-dashboard`: Builds from `infra/apps/smp/production/dashboard.Dockerfile` with `vars.VITE_API_BASE_URL`
   - `smoke-test`: Non-blocking validation using `smoke-test.sh` with generated temporary secrets

2. **`.github/workflows/release-please.yml`** — Wired to invoke `release-image.yml` after `smp` release creation using `smp--tag_name` output

3. **`infra/apps/smp/production/compose.yaml`** — Updated to default to GHCR registry images with `pull_policy: always`

4. **`infra/apps/smp/production/compose.override.yaml`** — Created to preserve local build option via override
...
**Decision**: Reuse existing Spring Boot buildpack configuration instead of creating a new Dockerfile.
...
- Troubleshooting guidance for workflow failures (GHCR push denied, buildpack timeout, smoke test flaky)
...
1. **Trigger the workflow**:
   - Merge to `main`
   - Wait for release-please to create or update the `smp@*` tag (or create one manually for testing)
