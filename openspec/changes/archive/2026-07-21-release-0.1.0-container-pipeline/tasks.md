...
- A release-please tag builds and pushes `ghcr.io/dallay/profiletailors-smp` and `ghcr.io/dallay/profiletailors-dashboard` with tag and `latest`.
...
If verification requires large workflow fixes, merge PR 3 as a size exception with maintainer approval.
