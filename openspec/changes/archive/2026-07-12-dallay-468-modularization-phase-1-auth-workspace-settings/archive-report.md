...
PASS WITH WARNINGS. No critical issues were present in `verify-report.md`, so the change is eligible
for archive.
...
1. `just frontend-test` and `just frontend-lint` currently run marketing recipes, not the Vue app
   touched by this change. App-focused Vitest and Biome commands were used as adequate verification
   and passed.
2. The raw changed-line count is high because this is a physical relocation. PR reviewers should use
   rename-aware diff settings.
...
The change has been planned, implemented, verified, synced into the OpenSpec source of truth, and
archived.
