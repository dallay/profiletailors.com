...
For SERVICE_ACCOUNT principals on the proving slice, the system MUST materialize the authenticated principal from the validated service-account bearer credential path.
For API_KEY principals on the proving slice, the system MUST materialize the authenticated principal from the validated API-key credential path.
...
Federated social login, external account linking, broader identity lifecycle breadth, and generalized multi-principal onboarding flows remain deferred.

(Previously: The system supported repo-local authenticated principal materialization for the proving slice and USER principals were materialized from validated JWTs, but no session-bootstrap or refresh-driven re-issuance requirement existed for browser sessions.)
