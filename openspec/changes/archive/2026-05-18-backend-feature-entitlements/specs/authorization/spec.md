...
(Previously: Feature entitlements were modeled separately from permissions, and capability evaluation MAY require both checks, but full entitlement management could remain deferred without executable proving-slice enforcement.)
...
(Previously: The proving slice executed persisted workspace-scoped direct grants on the existing endpoint and explicitly excluded entitlement-gated execution.)
...
(Previously: PostgreSQL verification covered direct-grant execution paths on the existing slice and explicitly excluded entitlement-related verification.)
