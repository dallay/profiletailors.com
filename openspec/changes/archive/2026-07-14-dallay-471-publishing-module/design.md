...
Relocate publishing-owned Vue SPA files into `apps/web/app/src/modules/publishing/` using the same
direct-move pattern as DALLAY-468/470. This is a behavior-preserving physical move plus import/mock
rewrite only: scheduler URLs, Pinia store IDs/state, composer workflow, media picker behavior, and
component props/events stay unchanged. Media state remains owned by `modules/media` from DALLAY-469;
publishing may import media only through stable `@modules/media/...` paths.
...
Start by extending `module-relocation.spec.ts` so new `@modules/publishing` and media-store imports
fail before files move. Then move files, rewrite imports/mocks, and make existing tests pass without
changing assertions.
...
Avoid broad `just ci` during relocation unless focused checks reveal cross-app risk or before final
push/PR.

## Migration / Rollout

No migration required. Roll out as one relocation PR. Rollback is a git revert of file moves/import
rewrites, then rerun the focused Vitest and type-check commands.
