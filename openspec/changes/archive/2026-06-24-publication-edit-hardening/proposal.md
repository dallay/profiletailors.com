...
The archived `edit-publication-composer` change shipped successfully, but verification surfaced three follow-up hardening items:

1. Pre-existing Detekt violations in `R2dbcPublishingRepositories.kt` (`LargeClass`, `LongMethod`) still block `just backend-check` / CI.
...
Harden the publication editing feature so the backend quality gate passes, edit-mode behavior is covered with focused unit tests, and the complete user flow is protected with E2E coverage.
...
1. Refactor persistence code by extracting cohesive helpers from `R2dbcPublishingRepositories.kt`, especially around `insertOrUpdate` and any large responsibility clusters causing Detekt.
2. Add focused unit tests for `CreatePostModal` edit mode branches previously marked partial in verification.
