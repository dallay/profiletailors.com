...
**And** the previous `LargeClass` and `LongMethod` findings for `R2dbcPublishingRepositories.kt`
MUST no longer fail the backend quality gate
...
**Then** dedicated tests MUST verify edit-mode prefill behavior for content, scheduling, priority,
media, and locked channel state
...
**Then** Playwright coverage MUST verify the detail modal closes, the composer opens in edit mode,
fields are pre-filled, the update succeeds, and the scheduler refreshes to show saved state
