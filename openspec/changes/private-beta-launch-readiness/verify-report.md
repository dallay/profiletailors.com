...
2. `just backend-bdd-postgres` and `:server:smp:postgresIntegrationTest` did not re-run in this
   verification (Docker was not started; `just backend-build` shows the Postgres tasks as
   `UP-TO-DATE` from a prior successful invocation). The Postgres integration test
