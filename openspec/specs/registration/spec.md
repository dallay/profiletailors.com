...
(Argon2id for new registrations). If any in-transaction mutation fails, the system MUST roll back
all registration mutations from that attempt. Partial user, credential, workspace, membership, role,
verification-token, or password algorithm metadata records MUST NOT remain committed after rollback.
