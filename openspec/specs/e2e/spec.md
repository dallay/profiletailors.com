...
The E2E login plan MUST assert a 12-character password minimum in every password-policy reference.
Section 5.4 ("Registration with short password") MUST expect error detail
`"Password must contain at least 12 characters."` (Previously: the plan asserted "at least 8
characters"; the working tree already updates §5.4 and Appendix B.)
...
The E2E register plan MUST assert a 12-character minimum everywhere. Section 1.3 and 8.1 (EN)
MUST show placeholder `"At least 12 characters"`; section 4.3 MUST expect 400 detail
`"Password must contain at least 12 characters."`. (Previously: plan asserted "At least 8
characters" in EN and stale `"Al menos 8 caracteres"` in the ES section 8.2 — the ES copy gap MUST
be closed to say `"Al menos 12 caracteres"`.)
...
acceptance scenarios when each status, application code, browser state, and required persistence
...
- GIVEN local fixtures for expired, revoked, mismatched, and matching-existing-identity
- WHEN the acceptance matrix runs
- THEN each scenario produces recorded evidence
- AND the report is labeled as local evidence, not deployed acceptance
