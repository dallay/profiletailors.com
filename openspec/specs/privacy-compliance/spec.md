...
The consent banner MUST present a non-modal first-visit summary with equal-prominence action
buttons,
...
**Equal prominence rule**: Summary actions MUST have identical size, color saturation, and position
hierarchy. No dark patterns (e.g., green accept + gray reject).
...
The banner MUST be theme-independent and always render in a dark palette regardless of the page
theme.
...
> Scope note (DALLAY-579, archived 2026-09-09): the eight requirements below govern the
> authenticated app (dashboard) non-modal consent banner surface (capability
> `consent-banner-presentation`). They supplement — not replace — the marketing-surface
> requirements above. The receipt/source/version contract (`pt-consent`) is unchanged. Where an
> app-surface rule differs from a marketing-surface rule (e.g. CookieSettings modal vs inline
> panel, light/dark themes vs always-dark), each applies only to its own surface.
...
> ⚠️ Outstanding operator acceptance (maintainer exception 2026-09-09, DALLAY-579): the manual
> Brave Shields ON/OFF × states A–D and Safari/WebKit × A–D (EN/ES, light/dark, 320/768/1280)
> matrix is NOT RUN — no operator harness exists in this environment, and WebKit automation is
> structurally excluded from dashboard E2E. The R7 orphaned-overlay failure class was removed
> structurally (zero dialog/portal/overlay primitives in the banner path, verified by component
> tests + `expectNoOverlay` E2E on chromium/firefox/Mobile Chrome), so residual risk is limited to
> non-blocking visual quirks. Re-run prerequisite: a human operator runs the design.md diagnosis
> checklist per state and records results before this warning is lifted. Carry this warning in
